                    ...
                    <tr class="align-top">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex min-w-[220px] items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    <p class="truncate text-sm font-semibold text-[#111827]">{{ $post->user->name }}</p>
                                    <p class="truncate text-xs text-[#7a8392]">{{ $post->user->email }}</p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                @if (! $post->trashed())
                                    @if ($post->is_flagged)
                                        <form method="POST" action="{{ route('admin.content.community-posts.unflag', $post) }}">
                                            @csrf
                                    ...
                                    @else
                                        <form method="POST" action="{{ route('admin.content.community-posts.flag', $post) }}">
                                            @csrf
                                    ...
                                    <form method="POST" action="{{ route('admin.content.community-posts.delete', $post) }}" onsubmit="return confirm('Are you sure you want to delete this post?')">
                                        @csrf
                                        @method('DELETE')
                                ...
                                @else
                                    <form method="POST" action="{{ route('admin.content.community-posts.restore', $post) }}">
                                        @csrf
