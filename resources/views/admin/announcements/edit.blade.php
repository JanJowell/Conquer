...
@section('content')
    <div class="mx-auto max-w-4xl space-y-6">
        <div>
            ...
            <h1 class="mt-2 text-3xl font-semibold tracking-tight text-[#151b26]">Edit Announcement</h1>
            <p class="mt-2 text-sm text-[#6d7685]">Update the notice content, event assignment, or publishing state.</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.announcements.update', $announcement) }}" enctype="multipart/form-data" class="rounded-2xl border border-[#d9dee7] bg-white p-6 shadow-sm">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-5">
                <div>
                    ...
                    <select id="event_id" name="event_id" class="h-12 w-full rounded-xl border border-[#d9dee7] px-4 text-sm text-[#151b26] outline-none transition focus:border-[#aeb7c3] focus:ring-2 focus:ring-[#eef1f5]">
                        ...
                        @foreach ($events as $event)
                            <option value="{{ $event->id }}" @selected((string) old('event_id', $announcement->event_id) === (string) $event->id)>{{ $event->title }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input id="title" name="title" type="text" value="{{ old('title', $announcement->title) }}" class="h-12 w-full rounded-xl border border-[#d9dee7] px-4 text-sm text-[#151b26] outline-none transition focus:border-[#aeb7c3] focus:ring-2 focus:ring-[#eef1f5]">
                </div>

                <div>
                    <label for="content" class="mb-2 block text-sm font-medium text-[#3d4757]">Content</label>
                    <textarea id="content" name="content" rows="6" class="w-full rounded-xl border border-[#d9dee7] px-4 py-3 text-sm text-[#151b26] outline-none transition focus:border-[#aeb7c3] focus:ring-2 focus:ring-[#eef1f5]">{{ old('content', $announcement->content) }}</textarea>
                </div>
                ...
                <label class="inline-flex items-center gap-3 rounded-xl border border-[#d9dee7] px-4 py-3 text-sm text-[#202733]">
                    ...
                    <input type="checkbox" name="is_published" value="1" @checked(old('is_published', $announcement->is_published)) class="h-4 w-4 rounded border-[#cfd5de] text-[#151b26] focus:ring-[#151b26]">
                    Published
                </label>

                <div>
                    <label for="expires_at" class="mb-2 block text-sm font-medium text-[#3d4757]">Expires At</label>
                    <input id="expires_at" name="expires_at" type="datetime-local" value="{{ old('expires_at', $announcement->expires_at?->format('Y-m-d\TH:i')) }}"
